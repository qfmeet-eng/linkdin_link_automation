...
def api_analyze_selected_profiles(request):
    ...
    for i, url in enumerate(urls_to_process):
        ...
        try:
            result = analyze_lead(url, search_url=keyword, user=request.user)
            if "error" in result:
...
def api_start_lead_analysis(request):
    ...
    if not search_url:
        return JsonResponse(
            {"success": False, "message": "LinkedIn search URL required."},
            status=400,
        )
    ...
    from .lead_analyzer import run_lead_analysis_thread

    try:
        analyzed = run_lead_analysis_thread(search_url, request.user.id)
        return JsonResponse({
            "success": True,
            "analyzed": analyzed,
